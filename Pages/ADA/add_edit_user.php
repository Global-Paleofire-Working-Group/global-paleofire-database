<?php
/* 
 * fichier Pages/ADA/add_edit_user.php 
 * 
 */ 
if (isset($_SESSION['started']) && (isset($_SESSION['gcd_user_role']) && (($_SESSION['gcd_user_role'] == WebAppRoleGCD::CONTRIBUTOR) || $_SESSION['gcd_user_role'] == WebAppRoleGCD::ADMINISTRATOR || $_SESSION['gcd_user_role'] == WebAppRoleGCD::SUPERADMINISTRATOR))) {
    ...
    $min_length_pwd = 6;
    $max_length_pwd = 15;//xli 20/6/16 longueur max passée de 10 à 15
    $id = null;
    ...
    if (isset($_GET['id'])) {
        // cas d'une édition, on récupère l'utilisateur en fonction de l'identifiant passé dans l'URL
         ...
         if (isset($_SESSION['gcd_user_role']) && 
            ...
            (($_SESSION['gcd_user_role'] == WebAppRoleGCD::CONTRIBUTOR) && ($id != null && $_SESSION['gcd_user_id'] == $id)))){
    ...
    if (isset($_SESSION['gcd_user_role']) && 
        ...
        if (isset($_SESSION['gcd_user_role']) && ($_SESSION['gcd_user_role'] == WebAppRoleGCD::ADMINISTRATOR || $_SESSION['gcd_user_role'] == WebAppRoleGCD::SUPERADMINISTRATOR)){    
            $redirection="?p=CDA/user_view&gcd_menu=CDA";
        } else {
            $redirection=" ";
        }
        ...
        if (isset($_POST['submitAdd'])) {
            ...
            echo '<div class="btn-toolbar" role="toolbar" style="float:left">';
                if($id != null && $_SESSION['gcd_user_id'] == $id) {
                    echo "<input type = 'button' name = '' onclick=\"redirection('index.php')\" value = 'Go back to index' />";
                } else {
                    echo "<input type = 'button' name = '' onclick=\"redirection('index.php".$redirection."')\" value = 'Go back to users' />";
                }    
            echo '</div>';
        }
            ...
            <form action="" class="form_paleofire" name="formAdd" method="post" id="formAdd" >
                <fieldset class="cadre">
                    ...
                    <p>
                        ...
                        <input type="password" name="<?php echo WebAppUserGCD::PASSWORDFIELD; ?>" id="pwd" maxlength="15" size="15" />
                    </p>
                    <p>
                        <label for="pwd2">Repeat the password</label>
                        <input type="password" name="<?php echo WebAppUserGCD::PASSWORDFIELD; ?>2" id="pwd2" maxlength="15" size="15" />
                    </p>
                    <?php 
                    if (isset($_SESSION['gcd_user_role']) && ($_SESSION['gcd_user_role'] == WebAppRoleGCD::ADMINISTRATOR||$_SESSION['gcd_user_role'] == WebAppRoleGCD::SUPERADMINISTRATOR)){
                    ?>
                ...
                <fieldset class="cadre">
                    ...
                    <p>
                        ...
                        connectionBaseInProgress();
                        $listContact = Contact::getListForSelect();
                        echo '<select name='.Contact::ID.' id="id_contact" >';
                        
                        foreach ($listContact as $values) {
                            if ($values[CONTACT::ID] == $id_selected) {
                                $select = " selected ";
                            } else {
                                $select = "";
                            }
                            echo '<option value="' . $values[CONTACT::ID] . '"'. $select . ">" . $values[CONTACT::LASTNAME].' '.$values[CONTACT::FIRSTNAME].' ['.$values[CONTACT::ID] . "]</option>";
                        }
                        echo '</select>';
                ...
                <p class="submit">
                    ...
                    echo "<input type = 'button' name = 'cancelAdd' onclick=\"redirection('index.php".$redirection."')\" value = 'Cancel' />";
                    ?>
                </p> 
            </form>
            <?php
